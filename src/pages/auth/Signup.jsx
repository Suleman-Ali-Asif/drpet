...
const Signup = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.data.succeeded) {
        toast.success(
          "You have successfully signed up. Please verify your email to continue."
        );
        setTimeout(() => {
  ...
  return (
    <div className="flex flex-col sm:flex-row  justify-between items-stretch min-h-screen main-wrapper">
      ...
      <div
        className="w-full ml-4 mt-16 sm:w-1/2 flex-grow bg-white flex 
                flex-col items-left py-10 px-10 rounded-bl-10xl"
      >
        <h2
          className="text-2xl sm:text-2xl md:text-2xl lg:text-3xl 
                text-left font-semibold mb-4"
        >
          Sign up
        </h2>
        <p className="text-gray-500">
          Sign up to DoctorPet.com to discover the best pet products, exclusive
          deals, and expert care tips tailored for your furry friends.
        </p>
